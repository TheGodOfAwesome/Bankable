...
export default function SellAssets(props: {
  ...
  const getStockPrice = async (symbol: string, apiKey: string) => {
    ...
    userAssets. map((ast: any)=>{
      if (ast.token == selectedValue) {
        const price = (Number(ast?.token_value) * Number(latestClosePrice)).toFixed(2)
        setStockUnits(-ast?.token_value);
        setPayout(Number(price));
      }
    });
  ...
  const processOrder = async () => {
    // if (stockPrice > 0 && stockUnits > 0 && selectedValue != "" && stockTicker != "" && payout > 0)  {
    if (payout > 0)  {
      ...
      toast({
        title: "Please select an Asset to sell!",
        status: "warning",
        isClosable: true,
      })
    } 
  ...
  function getFullAssetName(targetAsset: string): string | undefined {
    let match = "No match"
    assetNames.map((astName) => {
      if (astName.includes(targetAsset)){
        match = astName;
      }
    });
    return match;
  ...
  useEffect(() => {
    const fetchAssetData = async () => {
      ...
      const {data, error} = await supabase.from('transactions').select().eq('token_recipient', email);
      if (error) { 
    ...
    <Card p="20px">
      <Flex direction={{ base: 'column' }} justify="center">
        ...
        <Flex flexDirection="column" justify="space-between" h="100%">
          ...
          >
            <FormControl>
              <FormLabel>You are selling: {stockTicker}</FormLabel>
                <Select placeholder='Select Asset' value={selectedValue} onChange={(event) => {setETF(event.target.value)}}>
                  {userAssets?.map((userAsset: any) => (
                    <option key={userAsset.id} value={userAsset.token}>
                      {getFullAssetName(userAsset.ticker_symbol)}
                    </option>
                  ))}
                </Select>
            </FormControl>
          ...
          >
            <FormControl isInvalid={errorAmountState}>
              <FormLabel>You Receive: USD$ {payout}</FormLabel>
              <InputGroup>
                <InputLeftElement pointerEvents='none' color='gray.300' fontSize='1.2em'>
                  $
                </InputLeftElement>
                <Input placeholder='Payment' value={payout} isDisabled={inactivePaymentInput} isReadOnly onChange={(event) => {calculateOrder(Number(event.target.value))}}/>
              </InputGroup>
